...
async def qty_chosen(call: types.CallbackQuery, state: FSMContext):
    ...
    await safe_edit(call.message, "📄 Отправьте текст резюме или нажмите Пропустить:", reply_markup=resume_skip_kb())
    await state.set_state(SetupStates.waiting_for_resume)
    await call.answer()

# удалены обработчики для вакансии
