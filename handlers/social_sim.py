...
def type_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="😏 Игривый(ая)/кокетливый(ая)", callback_data="type_flirty")],
        [InlineKeyboardButton(text="🌸 Скромный(ая)/загадочный(ая)", callback_data="type_shy")],
        [InlineKeyboardButton(text="🧐 Прямолинейный(ая)/саркастичный(ая)", callback_data="type_sarcastic")]
    ])
...
async def type_chosen(call: types.CallbackQuery, state: FSMContext):
    ...
    # Определяем имя и род для склонений
    if gender == "gender_girl":
        persona_name = "Девушка"
        gender_adj = "ая"  # окончание для прилагательных (игривая, скромная)
        pronoun = "Ты"
        gender_word = "девушка"
    else:
        persona_name = "Парень"
    ...
    # Описания типажей в зависимости от пола
    if persona_type == "type_flirty":
        if gender == "gender_girl":
            ...
            type_traits = (
                "Ты игривый и кокетливый. Подшучиваешь, используешь смайлики 😉, говоришь с намёком. "
                "Любишь проверять, насколько собеседница уверена в себе. Отвечаешь коротко, с лёгкой недосказанностью."
            )
    elif persona_type == "type_shy":
        if gender == "gender_girl":
            type_traits = (
                "Ты скромная и немного загадочная. Отвечаешь коротко, с тёплой интонацией. "
            ...
            type_traits = (
                "Ты скромный и немного загадочный. Отвечаешь коротко, с тёплой интонацией. "
                "Делаешь паузы (многоточия…). Не любишь слишком прямых вопросов, можешь перевести тему. "
                "Показываешь интерес не сразу, а постепенно. В речи часто используешь слова 'наверное', 'может быть'."
            )
    else:  # type_sarcastic
        if gender == "gender_girl":
            ...
            type_traits = (
                "Ты прямолинейный и саркастичный. Шутишь остро, иногда на грани. "
                "Ставишь неудобные вопросы. Не терпишь фальши. Если скажут глупость, едко комментируешь. "
                "Говоришь отрывисто, без лишних слов."
            )
    ...
    persona_content = (
        ...
        "Не пиши  как ИИ. Отвечай коротко, не более 50 слов. "
        "Никогда не используй рубль и все что связано с Россией и упоминанием о ней."
    )
